...
export default function ProjectLibrary({ search, onSelect }: { search: string; onSelect: () => void }) {
  ...
  return <div className="project-library">
    ...
    {visible.map((project) => <article key={project.id} aria-current={project.id === activeProjectId ? "page" : undefined} className="group px-1 py-0.5">
      {editing === project.id ? <form className="flex items-center gap-1 px-2 py-1.5" onSubmit={(event) => { event.preventDefault(); void perform(async () => {
        await updateProject(project.id, { name }); setEditing(null);
      }); }}>
        <input aria-label="Project name" value={name} onChange={(event) => setName(event.target.value)} autoFocus className="w-full bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-md px-2 py-1 text-[13px] outline-none" />
        <button disabled={busy || !name.trim()} className="text-[12px] px-1.5 text-stone-600">Save</button>
        <button type="button" onClick={() => setEditing(null)} className="text-[12px] px-1.5 text-stone-400">Cancel</button>
        ...
        <details className="relative shrink-0">
          <summary aria-label={`Actions for ${project.name}`} className="list-none cursor-pointer text-stone-400 hover:text-stone-700 dark:hover:text-zinc-200 px-1.5 py-1 text-[13px] rounded-md [&::-webkit-details-marker]:hidden">···</summary>
          <div className="project-actions absolute right-0 z-30 w-40 forge-card p-1 shadow-lg">
            {project.trashed ? <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md hover:bg-stone-100 dark:hover:bg-zinc-800" disabled={busy} onClick={() => change(project.id, { trashed: false })}>Restore project</button> : <>
              <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md hover:bg-stone-100 dark:hover:bg-zinc-800" disabled={busy} onClick={() => beginRename(project)}>Rename</button>
              <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md hover:bg-stone-100 dark:hover:bg-zinc-800" disabled={busy} onClick={() => change(project.id, { favorite: !project.favorite })}>{project.favorite ? "Unfavorite" : "Favorite"}</button>
              <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md hover:bg-stone-100 dark:hover:bg-zinc-800" disabled={busy} onClick={() => change(project.id, { archived: !project.archived })}>{project.archived ? "Unarchive" : "Archive"}</button>
              <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md hover:bg-stone-100 dark:hover:bg-zinc-800" disabled={busy} onClick={() => perform(async () => {
                const copy = await duplicateProject(project.id);
                navigate(`/projects/${copy.id}`); onSelect();
              })}>Duplicate</button>
              <button className="w-full text-left px-2.5 py-1.5 text-[12px] rounded-md text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30" disabled={busy} onClick={() => change(project.id, { trashed: true })}>Move to trash</button>
            </>}
          </div>
        </details>
      </div>}
