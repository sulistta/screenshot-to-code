...
### Settings reference
Settings follows `design/ChatGPT Image 5 de set. de 2026, 15_19_16 (8).png`: an unobscured sidebar, full-width settings page, left section navigation and stacked provider rows, model table and execution defaults. Credentials expand from provider rows. Status labels distinguish saved configuration from successful connection tests. Primary/subagent defaults apply to newly created projects; the removed stack-preference control stays retired. The project controller remains mounted while its hidden content stops layout/animation work.
